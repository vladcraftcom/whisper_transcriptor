...
public sealed partial class MainWindowViewModel : ViewModelBase, IDisposable
{
    ...
    public MainWindowViewModel()
    {
        ...
        // По умолчанию ставим английский, как просили. При необходимости можно ввести auto/ru/и т.д.
        Language = "en";
        RemoveSilence = true;
        Status = "Готово.";
        Log = "";
    ...
    private async Task<string> TranscribeWavAsync(string wavPath, CancellationToken ct)
    {
        ...
        await foreach (var segment in processor.ProcessAsync(fs, ct))
        {
            ...
            // Защита от "бесконечной мантры" на тишине/шуме:
            // если одно и то же повторилось много раз подряд — считаем это зацикливанием и прекращаем.
    ...
    private async Task<string> ConvertToWav16kMonoAsync(string inputPath, CancellationToken ct)
    {
        ...
        // ffmpeg должен быть доступен в PATH (Windows/macOS/Linux).
        ...
        {
            // Если включено удаление тишины, а фильтр не поддерживается/ругается — делаем фоллбек без фильтра.
            if (RemoveSilence)
            {
        ...
        // Минимальная проверка, чтобы не кормить Whisper пустым/битым файлом.
        // WAV header обычно ~44 байта, поэтому < 1KB почти наверняка проблема/тишина.
    ...
    {
        // Приводим к формату, который стабильно ест whisper.cpp: WAV 16kHz mono 16-bit PCM.
        // Опционально удаляем тишину, чтобы избежать "галлюцинаций" на длинных паузах.
        var baseArgs = $"-y -i \"{inputPath}\" -ac 1 -ar 16000 -c:a pcm_s16le";
        if (!removeSilence)
            return $"{baseArgs} \"{outputWavPath}\"";

        // silenceremove может вести себя по-разному в разных сборках ffmpeg, поэтому сверху есть фоллбек.
        var filter =
    ...
    {
        // Нормализуем для сравнения повторов: trim, lower, схлопываем пробелы.
